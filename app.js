...
function home() { const canResume = state.characterId && state.step < questions.length && state.screen !== 'result'; return `<section class="screen hero">${brand()}<div class="hero-content"><div class="home-stats"><span>🔟 连续学习 ${profile.streak} 天</span><span>🐚 ${profile.xp} 枚贝壳</span></div>${dailyCard()}<img class="art" src="assets/lamb-wave.jpg" alt="挥手的贝壳小羊" fetchpriority="high" decoding="async"/><h1>和贝壳小羊开启一场心动学习</h1><p class="lead">八段互动剧情，八种 C++ 解题方式。<br>每一次选择，都会变成一段小小的代码冒险。</p>${canResume ? btn(`继续第 ${state.step+1} 章`,'resume') : btn('开始冒险','choose')}<div class="home-actions">${btn('查看学习档案','report','secondary')}</div><p class="footnote">本机保存进度 · 无需注册 · 不收集个人信息</p></div></section>`; }
